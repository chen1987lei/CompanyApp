...
-(void)moveScrolViewWithIndex:(NSInteger)aIndex{
    if (mItemInfoArray.count < aIndex) {
     ...
     //宽度小于kScreenWidth肯定不需要移动
    if (mTotalWidth <= kScreenWidth) {
        return;
    }
    ...
    if (vButtonOrigin >= 300) {
        if ((vButtonOrigin + 180) >= mScrollView.contentSize.width) {
            [mScrollView setContentOffset:CGPointMake(mScrollView.contentSize.width - kScreenWidth, mScrollView.contentOffset.y) animated:YES];
            return;
        }
